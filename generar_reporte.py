...
def main():
    ...
    if period_days:
        ...
        kpi_period_label = f"{meses_es[until_date.month - 1]} {until_date.year}"
        kpi_month_start = until_date.replace(day=1).strftime("%Y-%m-%d")
        last_reach = monthly["reach"][-1] if monthly["reach"] else 0
        last_spend = monthly["spend"][-1] if monthly["spend"] else 0
    ...
    spend_wa, spend_leads = fetch_campaign_cost_breakdown(
        account_id, token, kpi_month_start, until
    )
    ...
    kpi_since_date = datetime.strptime(kpi_month_start, "%Y-%m-%d")
    period_len_days = (until_date - kpi_since_date).days + 1
    ...
    output_file = output_dir / config.get("output_filename", "dashboard.html")
